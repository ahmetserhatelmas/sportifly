...
import { fields, formatPrice, getCatalogItem } from "@/lib/catalog";
import { site } from "@/lib/site";

export function generateStaticParams() {
  return fields.map((i) => ({ slug: i.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    title: item.title,
    description: `${item.title} — ${item.city} / ${item.district}. Saatlik ${formatPrice(item)}. ${item.description}`,
  };
}
...
export default async function FieldDetailPage({
  ...
  const { slug } = await params;
  const item = getCatalogItem(slug);
  if (!item || item.kind !== "field") notFound();
    ...
    <article className="bg-white">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-12 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="lg:pt-8">
          ...
          <p className="mt-1 text-sm text-ink-3">KDV dahil. Saatlik rezervasyon.</p>
          ...
          <a
            href={`mailto:${site.supportEmail}?subject=${encodeURIComponent(item.title + " rezervasyon")}`}
          ...
          <p className="mt-3 text-xs text-ink-3">
            Uygulama yayına alındığında rezervasyon uygulama içinden de yapılabilecek. Ödeme iyzico ile alınır.
          </p>
          <div className="mt-6">
            <PaymentLogos />
          </div>
